...
function decideMovement(closestPlayer, team, opposition, matchDetails) {
  ...
  let { position, Player, withPlayer, withTeam } = matchDetails.ball;
  for (const thisPlayer of team.players) {
    ...
    if (xPosition && yPosition && withTeam !== team.name) {
      if (
        thisPlayer.startPOS[0] === position[0] &&
        thisPlayer.startPOS[1] === position[1]
      ) {
        if (
          withPlayer === true &&
          ...
          if (action === `tackle`) {
            let foul = actions.resolveTackle(
              thisPlayer,
              team,
              opposition,
              matchDetails
    ...
    if (thisPlayer.hasBall === true) {
      ...
      if (ballActions.includes(action)) {
        ...
        if (action === `cleared` || action === `boot`) {
          ...
          let newPosition = ballMovement.ballPassed(
            matchDetails,
            team,
            thisPlayer
          );
          matchDetails.iterationLog.push(
            `passed to new position: ${newPosition}`
          );
          let tempPosition = newPosition.map((x) => x);
          matchDetails.ball.position = tempPosition;
          matchDetails.ball.position[2] = 0;
          ...
          matchDetails.iterationLog.push(
            `crossed to new position: ${newPosition}`
          );
          let tempPosition = newPosition.map((x) => x);
          matchDetails.ball.position = tempPosition;
          matchDetails.ball.position[2] = 0;
          ...
          let newPosition = ballMovement.shotMade(
            matchDetails,
            team,
            opposition,
            thisPlayer
...
function checkOffside(team1, team2, matchDetails) {
  ...
  const team2side =
    team2.players[0].originPOS[1] < pitchSize[1] / 2 ? `top` : `bottom`;
    ...
    if (ball.withTeam == team1.name && team1side == `top`) {
      ...
      for (let thisPlayer of team1.players) {
        ...
        if (
          common.isBetween(
            thisPlayer.startPOS[1],
            offsideYposition.pos1,
            offsideYposition.pos2
          )
        ) {
